...
if($data) {
	...
	if($a == 'edit'){
		...
		if($sql->set("UPDATE `lines` SET exclusive = ? WHERE docID = ? AND lineID = ?","iii", array($userID,$doc,$line))) {
			$last = $sql->set("INSERT INTO changes (writerID,docID,lineID,typ) VALUES (?,?,?,'block')","iii",array($userID,$doc,$line),true);
			echo 'true';
		}else{
	...
	if($a == 'add') {
		...
		if($data) {
			...
			$data = json_encode(array('text'=>$text,'newid'=>$dann));
			
			$last = $sql->set("INSERT INTO changes (writerID,docID,lineID,typ,data) VALUES (?,?,?,'add',?)","iiis",array($userID,$doc,$line,$data),true);
	...
	if($a == 'cat') {
		...
		if(isset($data[0])) {
			if($data[0]['exclusive'] == '') {
				...
				$last = $sql->set("INSERT INTO changes (writerID,docID,lineID,typ,data) VALUES (?,?,?,'remove',?)","iiis",array($userID,$doc,$line,$text),true);
	...
	if($a == 'save'){
		...
		if(isset($_GET['b']) && $_GET['b'] == 'unblock') {
			...
			$sql->set("INSERT INTO changes (writerID,docID,lineID,typ,data) VALUES (?,?,?,'change',?)","iiis",array($userID,$doc,$line,$text));
			$last = $sql->set("INSERT INTO changes (writerID,docID,lineID,typ) VALUES (?,?,?,'unblock')","iii",array($userID,$doc,$line),true);
		}else{
			$sql->set("UPDATE `lines` SET content = ? WHERE lineID = ? AND docID = ?","sii",array($text,$line,$doc));
		
			$last = $sql->set("INSERT INTO changes (writerID,docID,lineID,typ,data) VALUES (?,?,?,'change',?)","iiis",array($userID,$doc,$line,$text),true);
		}
		$sql->set("UPDATE docs SET lastChange = ? WHERE docID = ?","ii",array($last,$doc));
	}
	...
	if($a == ''){
		...
		if(isset($_GET['last']) && $_GET['last'] > 0) {
			...
			for($i=0;$i<$heartbeat;$i=$i+$steps) {
				if($data = $sql->get("SELECT lineID, typ, data ,changeID AS last FROM changes WHERE changeID > ? AND docID = ? AND NOT writerID = ? ORDER BY changeID","iii",array($last,$doc,$userID))) {
					$out = array();
					foreach($data as $d) {
